...
-- name_tr kolonunu ekle (bir kez çalıştır — varsa hata vermez):
ALTER TABLE public.market_chain_products ADD COLUMN IF NOT EXISTS name_tr TEXT;

-- match_group_id: aynı ürünü farklı marketlerde birleştirir (urun_eslestir.py tarafından doldurulur)
ALTER TABLE public.market_chain_products ADD COLUMN IF NOT EXISTS match_group_id TEXT;
CREATE INDEX IF NOT EXISTS idx_market_match_group ON public.market_chain_products(match_group_id) WHERE match_group_id IS NOT NULL;
